  ...
  return (
    <>
      <div className={'col-span-2 pt-[20px] md:px-[50px] px-[20px] shadow-lg'}>
        <h1 className="text-xl text-blue-950 font-bold mb-4">Contacts</h1>
        {users.map((user, index) => (
          <div className={'flex  py-[10px] gap-[25px]'}>
            <div className={'col-span-1 rounded-full'}>
              <img
                className={
                  'rounded-full md:h-[70px] md:w-[70px] h-[50px] w-[50px]'
                }
                src={UserImage}
                alt="UserImageLogo"
              />
            ...
            <div
              key={user.id}
              className={'grid gap-[10px] md:text-sm text-xs  text-blue-950 '}
            >
              <span>{user.name}</span>
